...
这里总结了在使用本项目时可能遇到的问题及其解决方案。本页面将逐步添加常见错误和问题的原因及处理方法，并基于具体案例详细解释解决方案，以支持您顺利使用软件。

## Live2D模型加载时的TypeError: \_currentFrameNo

::: warning 症状
在本项目中选择自制Live2D模型时，显示以下错误且模型无法渲染。
...
- 此外，以Cubism 5格式（.moc3 v4）导出的模型与本项目附带的库不兼容。
...
4. 将输出文件夹放置在`public/live2d/模型名称/`中，并在本项目中重新选择`模型名称.model3.json`。
